...
(function ($) {
    ...
    this.$element.tagsinput({
      ...
      tagClass: function (item) {
        if (self.validEmail(item)) {
          return "tag valid label label-keppler";
        }
        else {
          // alert('Email invalid')
          return "invalid";
  ...
  MultiEmail.prototype = {
    ...
    build: function () {
      ...
      this.$element.on('itemAdded', function (event) {
        if (!self.validEmail(event.item)) {
          ...
          self.$element.blur();
          $('.invalid').remove()
          $('#setting_messages_to').val(self.validItems)
        } else {
          self.validItems.push(event.item)
        }
        console.log(self.invalidItems)
        console.log(self.validItems)
      });
      ...
      this.container().on('keydown', 'input', function (event) {
        ...
        self.input().attr('size', self.input().val().length + 18);
      })
    ...
    remove: function (item) {
      return this.$element.tagsinput('remove', item);      
    },
    addPending: function () {
